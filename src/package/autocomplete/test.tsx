      ...
      <div {...props}>
         {state.inDialog ? (
            <Dialog open={state.inDialog} onClose={() => state.setInDialog(false)}>
               ...
               <DialogActions>
                  <Button onClick={() => state.setOptions(c_options)} children={`Laden`} />
                  <Button onClick={() => state.setInDialog(false)} color="primary" children={`Schließen`} />
               </DialogActions>
            </Dialog>
         ) : (
            <Autocomplete
         ...
         <ButtonGroup>
            <Button
               ...
               variant={state.variant === "standard" ? "contained" : undefined}
            >
               Standard
            </Button>
            <Button
               onClick={() => state.setVariant("outlined")}
               variant={state.variant === "outlined" ? "contained" : undefined}
            >
               Outlined
            </Button>
            <Button
               onClick={() => state.setVariant("filled")}
               variant={state.variant === "filled" ? "contained" : undefined}
            >
               Filled
            </Button>
         </ButtonGroup>
         <ButtonGroup style={{ marginLeft: 8 }}>
            <Button onClick={() => state.setOptions(c_options)}>Laden</Button>
            <Button onClick={() => state.setInDialog(true)} variant={state.inDialog ? "contained" : undefined}>
               Dialog
            </Button>
         </ButtonGroup>
         <ButtonGroup style={{ marginLeft: 8 }}>
            <Button onClick={() => state.setValue("René")} variant={state.value === "René" ? "contained" : undefined}>
               René
            </Button>
            <Button onClick={() => state.setValue("")} variant={!state.value ? "contained" : undefined}>
               Clear
            </Button>
         </ButtonGroup>
         <ButtonGroup style={{ marginLeft: 8 }}>
            <Button onClick={() => state.inputRef.focusEnd()}>Focus</Button>
            <Button onClick={() => state.inputRef.focusAll()}>Select</Button>
         </ButtonGroup>
      </div>
   );
});
